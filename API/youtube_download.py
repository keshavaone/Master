"""
YouTube downloader module for the GUARD application.

This module provides YouTube video downloading functionality that can be 
integrated into the main application without requiring authentication.
"""
...
from PyQt5.QtWidgets import (
    ...
    QRadioButton, QButtonGroup, QMessageBox, QSizePolicy, QApplication
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QObject, QTimer, QDateTime, QDateTime
        ...
        def install_thread():
            try:
                ...
                if process.returncode == 0:
                    ...
                    QTimer.singleShot(0, lambda: self.status_label.setText("yt-dlp installed successfully"))
                    QTimer.singleShot(0, lambda: self.download_btn.setEnabled(True))
                    QTimer.singleShot(0, lambda: self.progress_bar.setFormat("Ready"))
                else:
                    self.log(f"Failed to install yt-dlp: {process.stderr}")
                    # Update UI in main thread
                    QTimer.singleShot(0, lambda: self.status_label.setText("Failed to install yt-dlp. Please install it manually."))
                    QTimer.singleShot(0, lambda: self.download_btn.setEnabled(False))
            except Exception as e:
                self.log(f"Error installing yt-dlp: {str(e)}")
                # Update UI in main thread
                QTimer.singleShot(0, lambda: self.status_label.setText(f"Error installing yt-dlp: {str(e)}"))
                QTimer.singleShot(0, lambda: self.download_btn.setEnabled(False))
...
def integrate_youtube_downloader(main_window):
    ...
    yt_downloader = YouTubeDownloaderWidget(
        ...
        log_callback=lambda msg: main_window.update_log(
            main_window.assistant.get_current_time() if hasattr(main_window, 'assistant') and main_window.assistant is not None else 
...
if __name__ == "__main__":
    ...
    downloader = YouTubeDownloaderWidget(window)
