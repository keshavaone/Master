...
class DataItemEditDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        id_label.setStyleSheet(f"color: {ModernColors.TEXT_SECONDARY}; font-weight: bold;")
        ...
        category_label.setStyleSheet(f"color: {ModernColors.TEXT_SECONDARY}; font-weight: bold;")
        self.category_input = QLineEdit(self.item_data.get('Category', ''), self)
        ...
        type_label.setStyleSheet(f"color: {ModernColors.TEXT_SECONDARY}; font-weight: bold;")
    ...
    def add_pii_field(self, name, value):
        ...
        self.pii_input_fields.append((name_input, value_input, remove_button, field_layout))
        
        # Connect remove button
        remove_button.clicked.connect(lambda: self.remove_pii_field(self.pii_input_fields[-1]))
...
class DeleteConfirmationDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        icon_label.setPixmap(self.style().standardIcon(QStyle.SP_MessageBoxWarning).pixmap(48, 48))
...
class ModernDataDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.add_item_button.setIcon(self.style().standardIcon(QStyle.SP_FileDialogNewFolder))
        ...
        self.empty_message.setStyleSheet(f"color: {ModernColors.TEXT_MUTED}; font-size: 16px;")
    ...
    def add_item_card(self, category_panel, item_data):
        ...
        id_label.setStyleSheet(f"color: {ModernColors.TEXT_MUTED}; font-size: 12px;")
        ...
        try:
                        ...
                        for pii_item in pii_items:
                            if isinstance(pii_item, dict):
                                ...
                                if name and value:
                                    ...
                                    name_label.setStyleSheet(f"color: {ModernColors.TEXT_SECONDARY}; font-weight: bold;")
                                    
                                    value_label = QLabel(value, card_frame)
                                    value_label.setStyleSheet(f"color: {ModernColors.TEXT_PRIMARY};")
            ...
            # Show error message
            error_label = QLabel(f"Error displaying data: {str(e)}", card_frame)
        ...
        # Update item count
        count_label = category_panel.findChild(QLabel, "", Qt.FindChildrenRecursively)
    ...
    def update_item_data(self, item_data):
        ...
        try:
            ...
            if success:
                ...
                QMessageBox.warning(self, "Error", f"Failed to update item: {response}")
        except Exception as e:
            progress_dialog.close()
    ...
    def delete_item(self, item_data):
        """
        Show confirmation dialog and delete an item.
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                if success:
                    ...
                    QMessageBox.warning(self, "Error", f"Failed to delete item: {response}")
            except Exception as e:
                progress_dialog.close()
...
class CRUDHelper:
    ...
    def perform_operation(operation, data, agent=None, auth_service=None, auth_manager=None, logger=None):
        ...
        if operation == 'create':
            valid, error_msg = CRUDHelper.validate_required_fields(data, ['Category', 'Type', 'PII'], logger)
            if not valid:
                return False, error_msg
        ...
        if agent:
            try:
                ...
                elif isinstance(response, Exception) or (isinstance(response, dict) and 'error' in response):
                    error_msg = str(response) if isinstance(response, Exception) else response.get('error', str(response))
                    if logger:
        ...
        if auth_manager and hasattr(auth_manager, 'token'):
            try:
                if logger:
